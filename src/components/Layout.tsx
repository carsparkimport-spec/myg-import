...
// Préfixe d'univers ('/jp', '/eu' ou '') déduit de l'URL courante
function useVerticalBase(): string {
  const firstSegment = usePathname().split('/').filter(Boolean)[0] ?? '';
  return firstSegment === 'jp' || firstSegment === 'eu' ? `/${firstSegment}` : '';
}

const Header = () => {
  const { t } = useI18n();
  const base = useVerticalBase();
  return (
      ...
      <div className="container mx-auto px-4 flex items-center justify-between h-full">
        <div className="flex-shrink-0 flex items-center h-full">
          <Link href="/" className="flex items-center" aria-label="Accueil principal">
            <Logo variant="rect" className="h-14 md:h-16 w-auto" />
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div>
            ...
            <ul className="space-y-2">
              ...
              <li><Link href={`${base}/stock`} className="hover:text-white">{t('nav.stock')}</Link></li>
              <li><Link href={`${base}/importation`} className="hover:text-white">{t('nav.import')}</Link></li>
              <li><Link href={`${base}/simulateur`} className="hover:text-white">{t('nav.sim')}</Link></li>
              <li><Link href={`${base}/a-propos`} className="hover:text-white">{t('nav.about')}</Link></li>
              <li><Link href={`${base}/blog`} className="hover:text-white">{t('nav.blog')}</Link></li>
              <li><Link href={`${base}/cgv#toc`} className="hover:text-white">{t('nav.cgv')}</Link></li>
            </ul>
          </div>
          <div>
            ...
            <ul className="space-y-4">
              <li>
                <span className="block text-white font-medium">MYG Import</span>
