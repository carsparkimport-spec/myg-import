...
function Header() {
  ...
  return <header className="eu-header">
...
function VehicleCard({ vehicle }: { vehicle: typeof vehicles[number] }) {
  return <article className="eu-vehicle-card">
    <div className="eu-vehicle-image"><img src={`/__mockup/images/eu-home/${vehicle.image}`} alt={`${vehicle.make} ${vehicle.model}`} /></div>
    <div className="eu-vehicle-info"><h3>{vehicle.make} {vehicle.model}</h3><strong>{vehicle.price} €</strong><p>{vehicle.year} · {vehicle.mileage} km</p><p>Automatique</p><a href="#contact">Voir le véhicule</a></div>
  </article>;
}
...
function Reviews() {
  ...
  return <section className="eu-reviews" id="reviews"><div className="eu-section-inner">
    <div className="eu-eyebrow">— Avis clients —</div><h2>Ils nous font confiance</h2>
    <div className="eu-rating"><span>★★★★★</span> <small>5/5 · Noté excellent</small></div>
    <div className="eu-review-grid">{reviews.map((review, index) => <article className={`eu-review ${index === current ? "eu-review-active" : ""}`} key={review[0]}>
    ...
    <div className="eu-page" id="top">
      ...
      <main>
        ...
        <section className="eu-benefits" id="process"><div className="eu-section-inner">
          ...
          <p className="eu-stock-intro text-center mr-[8px]" style={{ maxWidth: 680, color: "#999", fontSize: 15, margin: "0 0 25px" }}>Les véhicules présentés illustrent les recherches que nous pouvons mener pour vous. La disponibilité évolue chaque jour selon les stocks européens.</p>
